...
try {
 ...
 foreach($id in 4000000001,4000000002) {
  $map=[IO.MemoryMappedFiles.MemoryMappedFile]::CreateNew("Local\FrostBridgeLaunchV1-$id",8)
  $view=$map.CreateViewAccessor(); $view.Write(0,[uint32]0x314C4246); $view.Write(4,[int]1)
  $maps+=,$map; $views+=,$view
 }
 ...
 if($views[0].ReadInt32(4) -ne 1 -or $views[1].ReadInt32(4) -ne 1) { throw 'Client initiated launch' }
 $views[1].Write(4,[int]0)
 $hostBridge.StandardInput.WriteLine('start'); $hostBridge.StandardInput.Flush()
 Start-Sleep -Milliseconds 500
 if($views[0].ReadInt32(4) -ne 1 -or $views[1].ReadInt32(4) -ne 0) { throw 'Unready client was launched' }
 $views[1].Write(4,[int]1)
 $hostBridge.StandardInput.WriteLine('start'); $hostBridge.StandardInput.Flush()
 Wait-Until { $views[0].ReadInt32(4) -eq 2 -and $views[1].ReadInt32(4) -eq 2 } 'both launch requests'
 foreach($view in $views) { $view.Write(4,[int]3) }
 $hostBridge.StandardInput.WriteLine('start'); $hostBridge.StandardInput.Flush()
 $guestBridge.StandardInput.WriteLine('start'); $guestBridge.StandardInput.Flush()
 ...
 if($views[0].ReadInt32(4) -ne 3 -or $views[1].ReadInt32(4) -ne 3) { throw 'Duplicate launch changed game state' }
 ...
 $hostLog; $guestLog
 'PASS: host authority, readiness rejection, retry, two launch requests, exactly-once dispatch, Unicode resource labels.'
} finally {
